...
const makeName = (idOrName) => {
  ...
  if (idOrName.length > 10) {
    const person = employees.find((elem) => elem.id === idOrName);
    const nome = `${person.firstName} ${person.lastName}`;
    obj[nome] = person.responsibleFor.map((i) => animals.find((j) => j.id === i).name);
  } else {
    const p = employees.find((elem) => elem.firstName === idOrName || elem.lastName === idOrName);
    const nome = `${p.firstName} ${p.lastName}`;
    obj[nome] = p.responsibleFor.map((i) => animals.find((j) => j.id === i).name);
  }
  return obj;
};
